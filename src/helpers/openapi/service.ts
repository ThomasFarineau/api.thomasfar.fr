...
export type SetupConfig = {
    title?: string; version?: string; description?: string; servers?: { url: string }[]; components?: {
...
export class OpenAPIService {
  ...
  private _defaultUI: string = "swagger-ui"

  public get defaultUI(): string {
    return this._defaultUI;
  }

  public set defaultUI(value: string) {
    this._defaultUI = value;
  }
  ...
  public addRoute(path: string, method: string, spec: any): void {
    ...
    this.openApiDoc.paths[path][method] = {
      operationId: _.camelCase([path, method].join("_")), ...spec
    };
  }
  ...
  public setup(config: SetupConfig): void {
    ...
    if (config.components) {
      this.openApiDoc.components = {
        ...this.openApiDoc.components, ...config.components
      };
    }
  }
}
